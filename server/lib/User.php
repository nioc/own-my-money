...
class User
{
    ...
    public function getTransactions($periodStart = 0, $periodEnd = null)
    {
        ...
        require_once $_SERVER['DOCUMENT_ROOT'].'/server/lib/DatabaseConnection.php';
        $connection = new DatabaseConnection();
        $query = $connection->prepare('SELECT `transaction`.*, `account`.`user` AS `accountOwner`, `account`.`hasIcon`, `account`.`label` AS `accountLabel`, `share` FROM `transaction` LEFT JOIN `transaction_user_dispatch` ON `transaction_user_dispatch`.`user` = :user AND `transaction_user_dispatch`.`transaction` = `transaction`.`id`, `account` WHERE `account`.`id`=`transaction`.`aid` AND (`account`.`user` =:user OR `account`.`id` IN (SELECT `account` FROM `account_holder` WHERE `user` =:user)) AND `datePosted` > :periodStart AND `datePosted` < :periodEnd;');
        $query->bindValue(':user', $this->id, PDO::PARAM_STR);
